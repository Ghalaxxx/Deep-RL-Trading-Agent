...
1. Select a symbol and show the live status panel. Call out the feed label, usually `Simulated live replay`. Optional yfinance mode can be enabled with `RL_TRADING_FEED_MODE=live`.
...
The dashboard uses WebSocket updates through `/ws/live/{ticker}`. If WebSocket is unavailable, use `/api/live/{ticker}` or `/api/dashboard` as a polling fallback.
...
- The live-feeling feed is simulated replay unless the UI says `Live market data`.
